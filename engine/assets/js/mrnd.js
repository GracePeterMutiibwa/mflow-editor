...
(function (root, factory) {
    ...
    var DEPS = [
        ...
        {
            /* Always load the light theme — no dual/dark variant */
            name: 'highlight.js CSS', kind: 'css',
    ...
    function _injectCSS(href, dataAttr) {
    ...
    function _loadAllDeps() {
        ...
        DEPS.forEach(function (dep) {
            if (dep.test()) return;
            if (dep.kind === 'css') { _injectCSS(dep.href, dep.name.includes('highlight') ? 'light' : null); }
    ...
    function _injectMermaidStyles() {
        ...
        s.textContent = [
            ...
            '.md-mermaid-dl-menu{position:fixed;min-width:130px;background:var(--mmc-bg);border:1px solid var(--mmc-border-mid);border-radius:10px;padding:.3rem;z-index:99999;box-shadow:0 4px 16px rgba(0,0,0,.10),0 1px 4px rgba(0,0,0,.06);}',
    ...
    function _renderMermaidDiagram(placeholderEl, definition) {
        ...
        card.querySelector('#' + uid + '_dlbtn').addEventListener('click', function (e) {
            e.stopPropagation();
            /* Remove any existing menu — on body, so query from document */
            var existing = document.getElementById(uid + '_dlmenu');
            if (existing) { existing.remove(); return; }
            var btn = card.querySelector('#' + uid + '_dlbtn');
            ...
            /* Attach to body so overflow:hidden on the card cannot clip it */
            menu.style.background = '#ffffff';
            menu.style.borderColor = '#c8c6c0';
            menu.style.color = '#6b6b6b';
            document.body.appendChild(menu);
            /* Position relative to the button using fixed coords */
            var btnRect = btn.getBoundingClientRect();
            var menuW = 160; /* conservative estimated width before paint */
            var spaceBelow = window.innerHeight - btnRect.bottom;
            var spaceRight = window.innerWidth - btnRect.left;
            ...
            /* Align right edge to button right, but clamp so it never leaves the viewport */
            var rightAligned = btnRect.right - menuW;
            menu.style.left = Math.max(8, Math.min(rightAligned, window.innerWidth - menuW - 8)) + 'px';
            menu.querySelector('#' + uid + '_dl_svg').addEventListener('click', function () { menu.remove(); _downloadSVG(inner, btn); });
            menu.querySelector('#' + uid + '_dl_png').addEventListener('click', function () { menu.remove(); _downloadPNG(inner, btn); });
            var outside = function (e) { if (e.target !== btn && !menu.contains(e.target)) { menu.remove(); document.removeEventListener('click', outside); } };
            setTimeout(function () { document.addEventListener('click', outside); }, 0);
        });

        card.querySelector('#' + uid + '_expand').addEventListener('click', function () { _openFullscreen(uid, definition); });

        /* Always light theme for mermaid diagrams */
        mermaid.initialize({
            startOnLoad: false, suppressErrorRendering: true, theme: 'default', securityLevel: 'loose',
    ...
    function _downloadPNG(innerEl, btn) {
        ...
        img.onload = function () {
            ...
            cnv.width = res.W * SCALE; cnv.height = res.H * SCALE;
            var ctx = cnv.getContext('2d');
            ctx.fillStyle = '#ffffff'; ctx.fillRect(0, 0, cnv.width, cnv.height);
            ctx.scale(SCALE, SCALE); ctx.drawImage(img, 0, 0, res.W, res.H);
            try {
                cnv.toBlob(function (blob) { _triggerDownload(blob, 'diagram.png'); _flashOK(btn, prev); }, 'image/png');
            } catch (e) {
    ...
    function _openFullscreen(uid, definition) {
    ...
    function _injectFlashcardStyles() {
        ...
        .fc-card{width:100%;position:relative;transform-style:preserve-3d;transition:transform .55s cubic-bezier(.45,.05,.55,.95);border-radius:16px;}
        .fc-card.fc-flipped{transform:rotateY(180deg);}
        .fc-face{position:absolute;inset:0;border-radius:16px;border:1px solid var(--fc-border);backface-visibility:hidden;-webkit-backface-visibility:hidden;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:3rem 2rem 2.5rem;gap:.85rem;overflow-y:auto;box-sizing:border-box;}
        ...
        .fc-sizer{position:relative!important;inset:auto!important;visibility:hidden;pointer-events:none;border:none!important;background:transparent!important;padding:3rem 2rem 2.5rem;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:.85rem;min-height:220px;width:100%;box-sizing:border-box;}
    ...
    function _renderFlashcard(el, cards) {
        ...
        var term = wrap.querySelector('#' + uid + '_term');
        var def = wrap.querySelector('#' + uid + '_def');
        ...
        function renderDots() {
            ...
            cards.forEach(function (_, idx) {
                ...
                dot.className = 'fc-dot' + (idx === current ? ' fc-dot-active' : '') + (seen[idx] ? ' fc-dot-seen' : '');
        ...
        function renderCard(idx, animate) {
            var c = cards[idx];
            term.textContent = c.frontText;
            def.textContent = c.backText;
    ...
    /* ══════════════════════════════════════════════════════════════════
       §9  BUILT-IN STYLES  (light-only — no dark media queries)
    ══════════════════════════════════════════════════════════════════ */
    ...
    var CSS = [
        ...
        '.md-code-header{display:flex;justify-content:space-between;align-items:center;padding:.55em .9em !important;background:#fff;border-bottom:1px solid #e8ecf0;border-radius:10px 10px 0 0;}',
    ...
    function MDRenderer(options) {
        ...
        /**
         * print(container, print_title, alert_handler)
         *
         * Opens a new isolated window containing only the target container's
         * content and triggers print from there. Uses window.open() instead of
         * an iframe so mobile browsers print the correct isolated document rather
         * than falling back to the host page.
         *
         * Mermaid SVGs are rasterised to PNG before printing. Each diagram PNG
         * is capped at a safe page height and given CSS constraints that prevent
         * it from being split across pages. If the PNG is taller than one page,
         * landscape orientation is also requested to maximise available space.
         *
         * @param {string|Element} container  id string or DOM element wrapping all rendered content.
         * @param {string}         print_title  Document title for the print window.
         * @param {object}         alert_handler  Object with a .display(msg, type) method.
        ...
        function print(container, print_title, alert_handler) {
            ...
            /* ── Step 1: deep-clone the container so the live DOM is untouched ── */
            var clone = root.cloneNode(true);

            /* ── Step 2: flatten flashcards in the clone to static print blocks ── */
            clone.querySelectorAll('.fc-wrap').forEach(function (wrap) {
                var scene = wrap.querySelector('.fc-scene');
                if (!scene) return;

                var termEl = wrap.querySelector('.fc-front-term');
                var defEl = wrap.querySelector('.fc-back-def');
            ...
            /* ── Step 4: rasterise mermaid SVGs → PNG using the live DOM ────────
               Max canvas height is capped at PRINT_MAX_H logical px (at SCALE×).
               This prevents any single diagram from ever exceeding one page and
               avoids ugly mid-pixel page tears. If the diagram is taller than
               TALL_THRESHOLD px after scaling, the print page is switched to
               landscape orientation to give the image maximum room.              */
            var SCALE = 2;
            var PRINT_MAX_H = 900;   /* logical px cap — ~one A4 portrait content area */
            var TALL_THRESHOLD = 600; /* above this → request landscape */
            ...
            var svgJobs = liveMermaidCards.map(function (liveCard, idx) {
                return new Promise(function (resolve) {
                    ...
                    try {
                        ...
                        /* Scale down proportionally if taller than the page cap */
                        if (H > PRINT_MAX_H) {
                            var ratio = PRINT_MAX_H / H;
                            W = Math.round(W * ratio);
                            H = PRINT_MAX_H;
                        }

                        /* Flag landscape if image is still tall after capping */
                        if (H > TALL_THRESHOLD) needsLandscape = true;
                        ...
                        tmpImg.onload = function () {
                            ...
                            printImg.style.cssText = [
                                ...
                                'width:auto',
                                'height:auto',
                                /* Hard cap at 90% of the page height — keeps it on one page */
                                'max-height:90vh',
                                'margin:1.5em auto',
                                'border-radius:8px',
                                'page-break-inside:avoid',
            ...
            /* ── Step 6: open isolated window and print ──────────────────────────
               window.open() is used instead of an iframe because mobile browsers
               (iOS Safari, Chrome Android) do not reliably print iframe content —
               they fall back to printing the host page instead. A new window
               contains only the target HTML and prints correctly on all platforms. */
            return Promise.all(svgJobs).then(function () {
                var styles = _collectStyles();
                var pageSize = needsLandscape ? 'A4 landscape' : 'auto';
                ...
                if (!printWin) {
                    ...
                    alert_handler.display('Print blocked by your browser. Please allow popups for this page and try again.', 'warning');
                    return;
                }
                ...
                doc.open();
                doc.write(
                    '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>transformed..' + _escHtml(title) + '</title>'
                    + '<style>'
                    ...
                    + '.ac-callout,.fc-wrap,.md-mermaid-card{page-break-inside:avoid;break-inside:avoid;}'
                    + '.md-table-wrap{overflow:visible!important;max-width:100%!important;}'
                    + '.md-table{white-space:normal!important;}'
                    + '.md-table th,.md-table td{white-space:normal!important;overflow:visible!important;}'
                    + 'img{page-break-inside:avoid;break-inside:avoid;max-height:90vh;width:auto;}'
                    + '</style>'
                    + '</head><body>'
                    + bodyHTML
        ...
        /* tiny html-escape used by print() */
